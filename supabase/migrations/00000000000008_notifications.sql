...
create or replace function public.handle_contents_moderation_notification() returns trigger as $$
declare notification_type notification_type;
begin 
 if new.status = 'pending' then notification_type := 'content_pending';
    insert into public.notifications (user_id, type, data)
    select
      p.id,
      'content_pending_moderation',
      jsonb_build_object('content_id', new.content_id)
    from public.profiles p
    where p.id != new.user_id
      and exists (
        select 1
...
elsif new.status = 'changes_requested' then notification_type := 'content_changes_requested';
elsif new.status = 'approved' then
		insert into public.notifications (user_id, type, data)
		values (
			new.user_id,
		...
		from public.profiles p
		where p.id != new.user_id;
elsif new.status = 'rejected' then notification_type := 'content_rejected';
end if;
...
after
insert on public.contents_moderation for each row execute function public.handle_contents_moderation_notification();
-- RLS policies
