<script lang="ts">
	...
	import ForumModerationActions from './forum-moderation-actions.svelte';
	import ForumModerationStatusCell from './forum-moderation-status-cell.svelte';

	export let contents: ThreadWithModeration[];
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			cell: ({ value, row }) => {
				if (row.isData()) {
					return createRender(ForumModerationStatusCell, {
						status: value,
					});
				}
				return value ?? '';
		...
		table.column({
			...
			header: '',
			cell: ({ value }) => {
				return createRender(ForumModerationActions, {
					id: value.id,
