<script lang="ts">
	...
	import ContentsModerationActions from './contents-moderation-actions.svelte';
	import ContentsModerationStatusCell from './contents-moderation-status-cell.svelte';

	export let contents: ContentWithModeration[];
	export let updateModerationForm: SuperValidated<Infer<UpdateModerationInfoSchema>>;
	let data = writable(contents);
	$: data.set(contents);
	...
	const columns = table.createColumns([
		...
		table.column({
			...
			cell: ({ value, row }) => {
				if (row.isData()) {
					return createRender(ContentsModerationStatusCell, {
						status: value,
					});
				}
				return value ?? '';
		...
		table.column({
			...
			header: '',
			cell: ({ value }) => {
				return createRender(ContentsModerationActions, {
					id: value.id,
